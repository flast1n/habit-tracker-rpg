...
user_tokens: dict = {}
# Відстеження, кого вже сповістили про смерть, щоб уникнути дублювання
...
async def login_password(message: types.Message, state: FSMContext):
    ...
    # передати токени та ідентифікацію користувача постійно
    ...
    # очищення попереднього стану сповіщення про смерть телеграм користувача
...
async def logout(message: types.Message):
    ...
    # очищення стану сповіщення про смерть для цього телеграм користувача
    notified_deaths.pop(message.from_user.id, None)
...
# aiohttp повідомлення від сайту про події (наприклад смерть гравця щоб синхронізувати з ботом)
async def notify_handler(request):
    ...
    # розпатчити обробку в окрему задачу щоб не блокувати відповіді сайту
...
async def main():
    # завантажити постійне сховище й заповнити токени в пам'яті
    load_store()
    ...
    # запуск HTTP сервера сповіщень
    await start_notify_server()

    scheduler.add_job(send_reminders, "cron", hour=21, minute=0)
    # перевірка повідомлень про смерть гравця кожні 10 секунд для синхронізації сайту з ботом

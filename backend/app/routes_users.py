...
def get_activity_log(limit: int = 50, current_user: dict = Depends(get_current_user)):
    """Завантажує журнал подій для поточного користувача з таблиці action_logs"""
    print("DEBUG CURRENT USER IS:", current_user, type(current_user))
    ...
    if not user_id:
        raise HTTPException(status_code=400, detail="Не вдалося визначити ID користувача")

    conn = get_connection()
    logs = conn.execute("""
        SELECT action AS message, created_at FROM action_logs 
        WHERE actor_id = ?
...
def update_my_achievements(
    ...
    # Витягання ID користувача 
    user_id = current_user["id"] if isinstance(current_user, dict) else current_user.id

    # Перетворення масиву досягнень на текст (JSON) для бази даних

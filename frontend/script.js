    ...
    habits = raw.map(h => ({
        ...
        try {
                ...
                activityLog = rawLog.map(log => ({
                    msg: log.message,
                    t: new Date(log.created_at).toLocaleTimeString("uk-UA", { hour:"2-digit", minute:"2-digit" })
                })).reverse();
            } catch (e) {
...
// рендер головного героя та його статусу
...
async function checkAchievements() {
    ...
    // Отримання списку вже відкритих досягнень
    ...
    ACHIEVEMENTS_DATA.forEach(a => {
        ...
        if (!owned.includes(a.id) && a.check(currentUser)) {
            owned.push(a.id); // Запис те що воно тепер є
    ...
    // Оновлення даних локально, щоб відразу відобразити
    currentUser.achievements = owned;
    renderAchievementsMini();
    renderAchievementsTab();

    // Якщо відкрив хтось нове досягнення то відправляємо його на бекенд
    if (isNewUnlocked) {
        try {
...
async function completeHabit(id) {
    ...
    try {
        ...
        // Оновлення currentUser з відповіді
        currentUser = await api("GET", "/auth/me");
...
// ──────────────────────────────────────
// час відліку до скидання звичок
// ──────────────────────────────────────
...
function startGame() {
    ...
    checkAchievements();
    checkDeathOnLoad();
    startDeathPolling(); // запуск фонового polling
...
function startDeathTimer() {
    ...
    function tick() {
        ...
        if (msLeft <= 0) {
            stopDeathTimer();
            tryRevive();  // спроба воскреснути кілька разів
            return;
        }
...
// ──────────────────────────────────────
// читання подій та ініціалізація
// ──────────────────────────────────────
...
function initEvents() {
    ...
    // ── демостраційні дані для швидкого входу
    ...
    // ── включення/вимкнення показу пароля
    ...
    // ── пошук та фільтри в адмін панелі
    ...
    // ── модальне вікно редагування користувача в адмін панелі
    ...
    // ── контроль за модальним вікном деталей користувача
    ...
    // ── модальне вікно підвищення рівня
    ...
    // ── кнопка очищення логованого журналу
    document.getElementById("clear-log-btn").addEventListener("click", () => {
        activityLog = [];
        renderLog();
    });

    // ── виключення модальних вікон при кліку поза ними
...
async function init() {
    ...
    if (hasSession) {
        await enterApp();  // Автовхід, якщо токен правильний
    } else {
        showView("auth-screen");
    }
}
